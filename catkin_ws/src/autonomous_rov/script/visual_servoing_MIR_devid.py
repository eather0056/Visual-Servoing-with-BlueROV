...
def trackercallback(data):
	global tracked_point_vs
	tracked_point_vs = data.data

def timer_callback(event, lambda_control=0.1):
	global desired_point_vs, tracked_point_vs
	desired_point = np.array(desired_point_vs)
	tracked_point = np.array(tracked_point_vs)
	if desired_point is not None and tracked_point is not None:
		error_vs, vcam_vs, vrobot_vs = compute_visual_servoing(desired_point, tracked_point, lambda_control)
		convert_and_send_robot_velocity(vrobot_vs)
		error_msg = Float64MultiArray(data=error_vs)
		error_pub.publish(error_msg)
		vcam_msg = Float64MultiArray(data=vcam_vs)
		vcam_pub.publish(vcam_msg)
		vrobot_msg = Float64MultiArray(data=vrobot_vs)
		vrobot_pub.publish(vrobot_msg)

def convert_and_send_robot_velocity(vrobot):
	vel = Twist()
...
def interactionMatrix(point, Z=1 ):
	x, y = point
	L = np.array([[-1/Z, 0, x/Z, x*y, -(1 + x**2), y], [0, -1/Z, y/Z, 1 + y**2, -x*y, -x]])
	return L


def compute_visual_servoing(desired_point, tracked_point, lambda_control, Z=1 , controlled_dof=[0,1,2,3,4,5]):
	error_vs = desired_point - tracked_point
	currentL = interactionMatrix(tracked_point, Z)
	desiredL = interactionMatrix(desired_point, Z)
	mixteL= (currentL + desiredL)/2
	L = mixteL
	reducedL = L[:, controlled_dof]
	#vcam_vs = -(lambda_control * np.linalg.pinv(reducedL) @ error_vs)
	vcam_vs = -(lambda_control * np.linalg.pinv(reducedL).dot(error_vs))
	...
	vrobot_vs = transform(vcam_vs, R_c, t_c)

	return error_vs, vcam_vs, vrobot_vs


def transform(v_c, R_c, t_c):
	v_linear = v_c[:3]
	v_angular = v_c[3:]
	t_skew = np.array([[0, -t_c[2], t_c[1]],
                       [t_c[2], 0, -t_c[0]],
                       [-t_c[1], t_c[0], 0]])
	upper_block = np.hstack((R_c, -R_c.dot(t_skew)))
	lower_block = np.hstack((np.zeros((3, 3)), R_c))
	T_vel = np.vstack((upper_block, lower_block))
	v_robot = T_vel.dot(np.hstack((v_linear, v_angular)))
	return v_robot
...
if __name__ == '__main__':
	...
	rospy.Subscriber("desired_point", Float64MultiArray, desiredpointscallback, queue_size=1)
	subscriber()

	error_pub = rospy.Publisher('visual_servoing/error', Float64MultiArray, queue_size=10)
	vcam_pub = rospy.Publisher('visual_servoing/vcam', Float64MultiArray, queue_size=10)
	vrobot_pub = rospy.Publisher('visual_servoing/vrobot', Float64MultiArray, queue_size=10)
	rospy.Timer(rospy.Duration(0.01), timer_callback)
	# Visual Servoing
	# lambda_control = 0.1
	# error_vs, vcam_vs, vrobot_vs = compute_visual_servoing(desired_point_vs, tracked_point_vs, lambda_control)
	# convert_and_send_robot_velocity(vrobot_vs)
	#
	# error_msg = Float64MultiArray(data=error_vs)
	# error_pub.publish(error_msg)
